...
void runTest(int argc, char** argv)
{
    ...
    float * d_A;
    HANDLE_ERROR(cudaMalloc((void**) &d_A, mem_size_A));

    // copy host memory to device
    HANDLE_ERROR(cudaMemcpy(d_A, h_A, mem_size_A, cudaMemcpyHostToDevice) );
   ...
   // CUT_SAFE_CALL(cutStartTimer(timer));
    ...
    HANDLE_ERROR(cudaMemcpy(h_C, d_A, mem_size_A, cudaMemcpyDeviceToHost) );
