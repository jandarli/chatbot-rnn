...
void runTest(int argc, char** argv)
{
    ...
    float * d_A;
    cudaCheckErrors(cudaMalloc((void**) &d_A, mem_size_A));

    // copy host memory to device
    cudaCheckErrors(cudaMemcpy(d_A, h_A, mem_size_A, cudaMemcpyHostToDevice) );
   ...
   // CUT_SAFE_CALL(cutStartTimer(timer));
    ...
    cudaCheckErrors(cudaMemcpy(h_C, d_A, mem_size_A, cudaMemcpyDeviceToHost) );
